...
public class CompanionSpeechBubble extends AbstractGameEffect {
    ...
    public CompanionSpeechBubble(float x, float y, String msg, boolean isPlayer) {
        this(x, y, 2.0F, msg, isPlayer);
    }

    public CompanionSpeechBubble(float x, float y, float duration, String msg, boolean isPlayer) {
    ...
    private void updateScale() {
        ...
        this.scale = Interpolation.swingIn.apply(Settings.scale, Settings.scale / 4.0F, this.scaleTimer / 0.3F);
    }

    public void render(SpriteBatch sb) {
        this.shadowColor.a = this.color.a / 4.0F;
        sb.setColor(this.shadowColor);
        sb.draw(ImageMaster.SPEECH_BUBBLE_IMG, this.x - 128.0F + this.shadow_offset, this.y - 128.0F + this.wavy_y - this.shadow_offset, 128.0F, 128.0F, 256.0F, 256.0F, this.scale, this.scale, this.rotation, 0, 0, 512, 512, false, false);
        sb.setColor(this.color);
        sb.draw(ImageMaster.SPEECH_BUBBLE_IMG, this.x - 128.0F, this.y - 128.0F + this.wavy_y, 128.0F, 128.0F, 256.0F, 256.0F, this.scale, this.scale, this.rotation, 0, 0, 512, 512, false, false);
    }
