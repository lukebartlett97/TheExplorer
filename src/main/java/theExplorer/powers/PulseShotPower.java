...
public class PulseShotPower extends ExplorerPower {
    public AbstractCreature source;

    public static final String POWER_ID = ExplorerMod.makeID(PulseShotPower.class.getSimpleName());
    ...
    public PulseShotPower(final AbstractCreature owner, final AbstractCreature source, int amount) {
        name = NAME;
        ID = POWER_ID;
    ...
    public void atStartOfTurn() {
        for (int i = 0; i < amount; i++) {
            addToBot(new MakeTempCardInHandAction(new PulseShot(), 1));
        }
    }
}
