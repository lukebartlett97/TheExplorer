...
public class UpgradedPitTrapPower extends ExplorerPower {
    public AbstractCreature source;

    public static final String POWER_ID = ExplorerMod.makeID(UpgradedPitTrapPower.class.getSimpleName());
    ...
    public UpgradedPitTrapPower(final AbstractCreature owner, final AbstractCreature source, int amount) {
        name = NAME;
        ID = POWER_ID;
    ...
    public int onAttacked(DamageInfo info, int damageAmount) {
        if (info.owner != null && info.type == DamageInfo.DamageType.NORMAL && amount > 0) {
            addToBot(new ApplyPowerAction(info.owner, owner, new StunnedPower(info.owner, owner, 1)));
            amount -= 1;
            if (amount == 0) {
