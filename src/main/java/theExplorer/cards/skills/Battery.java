...
public class Battery extends CustomCard {
    ...
    public static final String ID = ExplorerMod.makeID(Battery.class.getSimpleName());
    ...
    private void removeOtherCopiesFromGroup(CardGroup cardGroup) {
        ArrayList<AbstractCard> cardsToRemove = new ArrayList<>();
        for (AbstractCard card : cardGroup.group) {
            if (card.cardID.equals(Battery.ID)) {
                cardsToRemove.add(card);
            }
        }
        for (AbstractCard removal : cardsToRemove) {
            cardGroup.removeCard(removal);
        }
    }
}
